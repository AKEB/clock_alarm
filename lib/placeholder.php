...
//@define('PLACEHOLDER_NEED_ADDSLASHES', !ini_get('magic_quotes_gpc'));
@define('PLACEHOLDER_NEED_ADDSLASHES', 1);
	...
	foreach ($compiled as $num=>$e) {
		...
		do {
			...
			if ($type === '') {
				...
				$repl = "'".(PLACEHOLDER_NEED_ADDSLASHES ? addslashes($a): $a)."'";
				break;
			}
			...
			if ($type === '@') {
				// Это список.
				foreach ($a as $v)
					$repl .= ($repl===''? "" : ",").(isset($v) ? "'".(PLACEHOLDER_NEED_ADDSLASHES ? addslashes($v): $v)."'": 'NULL');
			} elseif ($type === '%') {
				...
				foreach ($a as $k=>$v) {
					...
					$repl .= ($repl===''? "" : ", ")."`".$k."`=".(isset($v) ? "'".(PLACEHOLDER_NEED_ADDSLASHES ? addslashes($v): $v)."'": 'NULL');
				}
